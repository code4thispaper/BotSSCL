...
def user_extracted_features(profile_file, date):
    ...
    profile_info['description_entropy'] = calculate_entropy(
        data['description'])
    profile_info['description_sentiment'] = SentimentIntensityAnalyzer(
    ).polarity_scores(data['description'])['compound']
    ...
    #  "clean_description": ''.join(filter(lambda x: x in string.printable, data['description'])),
    return profile_info
...
if __name__ == "__main__":
    ...
    df.to_csv(f"profile_raw.csv", index=False)

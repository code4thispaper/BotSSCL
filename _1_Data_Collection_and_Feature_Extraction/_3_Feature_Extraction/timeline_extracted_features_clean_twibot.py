...
def generate_df(data):
    ...
    for tweet in data:
        created_at_time = datetime.strptime(
            tweet['created_at'], "%Y-%m-%d %H:%M:%S+00:00").timestamp()
        created_at.append(created_at_time)
        languages.append(tweet['lang'])
        tweets.append(tweet['text'])
        no_favourites.append(tweet['public_metrics']['like_count'])
        no_retweets.append(tweet['public_metrics']['retweet_count'])
        # Count
        if 'etities' in tweet:            
...
def tweet_extracted_features(timeline_file, date_scraped):
    ...
    # User Features
    timeline_info['favourites_count'] = df['favourites_count'].mean()
...
if __name__ == "__main__":
    ...
    df.to_csv("timeline_clean_twibot.csv", index=False)

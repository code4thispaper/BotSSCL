...
def tweet_extracted_features(df, tweet_index):

    timeline_info = deepcopy(TIMELINE_INFO)
    timeline_info['id'] = id
    timeline_info['dataset'] = 'gilani-2017'
    timeline_info['tweet_index'] = tweet_index
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame()
    read_df = pd.read_csv(f"{dataset}-sample-modified-tweets.csv")
    print(read_df.columns)
    for id in read_df['id'].unique():
        date_scraped = '2023-03-07'
        special_df = read_df[read_df['id'] == id]
        for i in range(special_df.shape[0] + 1):
            special_df['tweet'] = special_df['rewritten_tweet'].iloc[:i].tolist() + special_df['original_tweet'].iloc[i:].tolist()
            feature_per_user = tweet_extracted_features(special_df, i)
            print(f"{id}, {i} Extracted")
            df_dictionary = pd.DataFrame([feature_per_user])
            df = pd.concat([df, df_dictionary], ignore_index=True)
    df.to_csv(f"{dataset}_sample_modified_single_tweets_timeline_clean.csv", index=False)

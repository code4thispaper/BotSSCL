...
def days_between(d1, d2):
    d1 = datetime.strptime(d1, "%Y-%m-%dT%H:%M:%S.000Z")
...
def hours_between(d1, d2):
    d1 = datetime.strptime(d1, "%Y-%m-%dT%H:%M:%S.000Z")
    d2 = datetime.strptime(d2, "%Y-%m-%d")
    d2 = datetime(d2.year, d2.month, d2.day, 0, 0, 0)
    diff = abs(d2 - d1)
...
def user_extracted_features(profile_file, date):
    ...
    profile_info['id'] = timeline_file.split('/')[-2]
    profile_info['dataset'] = timeline_file.split('/')[-4]
    ...
    profile_info['user_age_hours'] = hours_between(data['created_at'], date)
    ...
    profile_info['username'] = data['username']
    clean_desc = cleanString(data['description'])
    profile_info['description'] = data['description']
    ...
    # Default profile
    try:
        profile_info['default_profile_user'] = int(data['default_profile'])
    except:
        profile_info['default_profile_user'] = 0
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame()
    for user in users:
        timeline_file = user + "/profile.json"

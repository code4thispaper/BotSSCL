...
from scarf.model_botsscl_aug_2 import BotSSCL

from example.dataset import TwitterAccountDatasetAug2
from example.utils import fix_seed, train_epoch, dataset_embeddings


def run(dataset, input_embedding_dimension, output_embedding_dimension):
    ...
    # Create directories
    dirs = ["aug2/loss", "aug2/tsne", "aug2/cm", "aug2/pretrained_model"]
    for dir in dirs:
    ...
    with open(f'Data/{dataset.split("-")[0]}_test_{input_embedding_dimension}_with_tweets.npy', 'rb') as f:
        X = np.load(f)
    with open(f'Data/{dataset.split("-")[0]}_test_{input_embedding_dimension}_with_tweets_0.3_corrupted.npy', "rb") as f:
        X_imputed = np.load(f)
    
    # Add data together
    X_total = np.append(X, X_imputed, axis=1)
    ...
    train_data, test_data, train_target, test_target = train_test_split(
        X_total, target, test_size=0.2, random_state=seed
    )
    
    # Convert to dataframe
    train_imputed = train_data[:, X.shape[1]:X.shape[1] + X_imputed.shape[1]]
    test_imputed = test_data[:, X.shape[1]:X.shape[1] + X_imputed.shape[1]]
    
    # Generate for view 2
    train_data = pd.DataFrame(train_data[:, : X.shape[1]])
    test_data = pd.DataFrame(test_data[:, : X.shape[1]])

    # to torch dataset
    train_ds = TwitterAccountDatasetAug2(
        train_data, 
        train_target,
        train_imputed
    )
    test_ds = TwitterAccountDatasetAug2(
        test_data, 
        test_target,
        test_imputed
    )
    ...
    torch.save(model, f"aug2/pretrained_model/model_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.pt")
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    fig.savefig(f'aug2/loss/loss_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    fig.savefig(f'aug2/tsne/tsne_train_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    plt.legend()
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug2/tsne/tsne_test_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug2/tsne/tsne_train_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug2/tsne/tsne_test_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    with open(f"aug2/{dataset}_{input_embedding_dimension}_{output_embedding_dimension}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    for i in range(len(dimensions)):
        for k in range(len(dimensions)):
            run(dataset, dimensions[i], dimensions[k])

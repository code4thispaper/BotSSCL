...
def run(dataset, rep, target, train_index, test_index, input_embedding_dimension, output_embedding_dimension, cross_val_index, no_folds, method, seed):

    # In[ ]:
    train_data = rep[train_index]
    test_data = rep[test_index]
    ...
    model = BotSSCL(
        ...
        corruption_rate=0.6,    #0.6 original
    ).to(device)
    ...
    torch.save(model, f"aug1/pretrained_model/model_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.pt")
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    fig.savefig(f'aug1/loss/loss_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.png', bbox_inches='tight', dpi=300)
    ...
    fig.savefig(f'aug1/tsne/tsne_train_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.png', bbox_inches='tight', dpi=300)
    plt.legend()
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug1/tsne/tsne_test_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug1/tsne/tsne_train_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug1/tsne/tsne_test_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}_{cross_val_index}_{no_folds}_{method}_{seed}.png', bbox_inches='tight', dpi=300)
    ...
    with open(f"aug1/{dataset}_{input_embedding_dimension}_{output_embedding_dimension}_{cross_val_index}_{no_folds}_{method}_{seed}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    dataset = sys.argv[1]
    input_embedding_dimension = int(sys.argv[2])
    output_embedding_dimension = int(sys.argv[3])
    no_folds = int(sys.argv[4])
    method = sys.argv[5]
    seed = int(sys.argv[6])
    ...
    if method == 'normal':
        skf = KFold(n_splits=no_folds, shuffle=True, random_state=seed)
    else:
        skf = StratifiedKFold(n_splits=no_folds, shuffle=True, random_state=seed)

    for i, (train_index, test_index) in enumerate(skf.split(rep, target)):
        run(dataset, rep, target, train_index, test_index, input_embedding_dimension, output_embedding_dimension, i, no_folds, method, seed)

...
def run(folder):
    ...
    df['Input Embedding Dimension'] = input
    ...
    df.to_csv(f"{folder}_results.csv", index=False)

    # Generate heat maps
    for d in ["varol-icwsm", "gilani-2017"]:
        generate_heat_maps(df, d, folder)


if __name__ == "__main__":

    folders = ["aug1", "aug2", "aug3"]
    for f in folders:
        run(f)

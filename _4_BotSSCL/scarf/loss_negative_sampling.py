...
class NTXent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        # Copy, sort and reduce similarity to 50%
        mask = (~torch.eye(batch_size * 2, batch_size * 2, dtype=torch.bool)).float().to(torch.device("cuda"))
        numerator = torch.exp(positives / self.temperature)
        similarity_sorted = torch.clone(similarity)
        similarity_sorted = mask * similarity_sorted
        similarity_sorted.sort(descending=False, dim=1)

        # Sort denominator and remove 50% of lowest similarities
        similarity_sorted[int(similarity_sorted.shape[0]/2):, :] = 0
        denominator = torch.exp(similarity_sorted / self.temperature)

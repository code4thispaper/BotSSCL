...
from scarf.model_botsscl_aug_3 import BotSSCL

from example.dataset import TwitterAccountDatasetAug3
...
def run(dataset, input_embedding_dimension, output_embedding_dimension):
    ...
    # Create directories
    dirs = ["aug3/loss", "aug3/tsne", "aug3/cm", "aug3/pretrained_model"]
    for dir in dirs:
    ...
    # to torch dataset
    train_ds = TwitterAccountDatasetAug3(
        train_data, 
        train_target
    )
    test_ds = TwitterAccountDatasetAug3(
        test_data, 
        test_target
    )
    ...
    torch.save(model, f"aug3/pretrained_model/model_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.pt")
    ...
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
    fig.savefig(f'aug3/loss/loss_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    fig.savefig(f'aug3/tsne/tsne_train_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    plt.legend()
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug3/tsne/tsne_test_embeddings_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug3/tsne/tsne_train_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    ax.scatter(reduced[~positive, 0], reduced[~positive, 1], label="negative")
    plt.legend()
    fig.savefig(f'aug3/tsne/tsne_test_data_{input_embedding_dimension}x{output_embedding_dimension}_{dataset}.png', bbox_inches='tight', dpi=300)
    ...
    with open(f"aug3/{dataset}_{input_embedding_dimension}_{output_embedding_dimension}.json", "w") as f:

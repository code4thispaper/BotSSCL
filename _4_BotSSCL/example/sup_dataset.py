...
class ExampleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        random_target = torch.tensor(self.target[random_idx], dtype=torch.float)

        sample = torch.tensor(self.data[index], dtype=torch.float)    
        sample_target = torch.tensor(self.target[index], dtype=torch.float)
        
        return sample, random_sample, sample_target

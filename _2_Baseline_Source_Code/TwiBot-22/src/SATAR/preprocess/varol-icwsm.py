...
parser.add_argument('--dataset', type=str, default='varol-icwsm')
...
def calc_activate_days(created_at):
    ...
    crawl_date = crawl_date.replace(tzinfo=None)
...
def get_properties():
    ...
    with open(osp.join(path, 'node_satar.json')) as f:
...
def get_word2vec_model():
    ...
    model = word2vec.Word2Vec(sentences, vector_size=128, workers=8, min_count=1) #Changed min_count

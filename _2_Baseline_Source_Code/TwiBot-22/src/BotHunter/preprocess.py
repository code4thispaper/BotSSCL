...
def calc_age(created_at):
    ...
    # Create new consistent mode
    if dataset == "gilani-2017":
        mode = '%Y-%m-%dT%H:%M:%S.000Z'
        collect_time = datetime.strptime('2023 Mar 07', '%Y %b %d')
    elif dataset == 'varol-icwsm':
        mode = '%Y-%m-%dT%H:%M:%S.000Z'
        collect_time = datetime.strptime('2023 Mar 05', '%Y %b %d')
    elif dataset in ['twibot-s1', 'twibot-s2']:
        mode = '%Y-%m-%d %H:%M:%S+00:00'
        collect_time = datetime.strptime('2022 Mar 15', '%Y %b %d')
    else:
        raise KeyError
